...
# Each modifier now includes a key "modifier_type": either "2D" or "3D".
# For 3D modifiers, the effective region is computed per layer.
modifier_defs = [
...
def compute_multiplier_for_modifier(x, y, z, mod):
    ...
    For "3D" modifiers, compute the effective XY region at the current Z.
    ...
    if mod["modifier_type"] == "2D":
        ...
        # Check vertical influence: effective influence only if z is within [cz - r_max, cz + r_max]
        if z < cz - mod["r_max"] or z > cz + mod["r_max"]:
            return 1.0
        # Compute effective XY radius:
...
def compute_effective_multiplier(start, end, z, num_samples=5):
    ...
    For 2D modifiers, use the union of modifier polygons to determine if both endpoints are in the same region.
    For 3D modifiers, the effective region is computed per layer.
...
for mod in modifiers:
    if mod.get("modifier_type", "2D") == "2D":
        ...
        # For 3D modifiers, draw the full mesh as a wireframe.
        ...
        if faces is not None:
            for face in faces:
                ...
                ax.plot(pts[:,0], pts[:,1], pts[:,2], color="blue", linewidth=1)
        else:
            # Fallback: plot the 2D convex hull.
            x_poly, y_poly = mod["polygon"].exterior.xy
            ax.plot(x_poly, y_poly, zs=z_ref, zdir="z", color="blue", linewidth=2, label="3D Modifier")
ax.legend()
set_axes_equal(ax)
