...
# Specify "modifier_type" as "2D" or "3D". For 3D modifiers, the influence is computed using the full 3D sphere.
modifier_defs = [
    {"filename": "modifier1.stl", "modifier_type": "3D", "center_multiplier": 1.5, "edge_multiplier": 1.5, "gradient_exponent": 1.0, "min_layer": 0.0},
...
def compute_multiplier_for_modifier(x, y, z, mod):
    ...
    For "3D" modifiers, compute an effective XY radius based on the current z.
    ...
    if mod["modifier_type"] == "2D":
        ...
        if z < cz - mod["r_max"] or z > cz + mod["r_max"]:
            return 1.0
        # Compute effective XY radius at this z:
        r_eff = math.sqrt(max(0, mod["r_max"]**2 - (z - cz)**2))
...
def compute_multiplier_multiple(x, y, z):
    """
    For a given (x, y, z) point, compute the overall extrusion multiplier
    by multiplying contributions from all modifiers.
    """
    overall = 1.0
...
def compute_average_multiplier(start, end, z, num_samples=5):
    """
    Sample evenly along the move (from start to end) and compute the average multiplier.
    """
...
def compute_effective_multiplier(start, end, z, num_samples=5):
    """
    Compute the effective extrusion multiplier for the move from start to end.
    For 2D modifiers, check if both endpoints are in the same region.
    For 3D modifiers, the effective influence is computed per layer.
    If both endpoints are in the same region, return the average multiplier along the move;
    ...
    if applicable_polys:
        ...
        start_inside = end_inside = True  # if no 2D modifiers, no conflict.
...
with open(input_file, "r") as fin, open(output_file, "w") as fout:
    for line in fin:
        # Handle G92 commands (reset extrusion).
        ...
        # Process G1 moves with extrusion.
...
for mod in modifiers:
    if mod.get("modifier_type", "2D") == "2D":
        ...
        # For 3D modifiers, compute the effective influence region at a given z level.
        cx, cy, cz = mod["centroid_3d"]
        # For visualization, we show the effective XY circle at the reference Z.
        z_level = z_ref
        if z_level < cz - mod["r_max"] or z_level > cz + mod["r_max"]:
            continue  # Not in vertical influence.
        r_eff = math.sqrt(max(0, mod["r_max"]**2 - (z_level - cz)**2))
        theta = np.linspace(0, 2 * math.pi, 100)
        x_circle = cx + r_eff * np.cos(theta)
        y_circle = cy + r_eff * np.sin(theta)
        ax.plot(x_circle, y_circle, zs=z_level, zdir="z", color="blue", linewidth=2, label="3D Modifier")
ax.legend()
set_axes_equal(ax)
