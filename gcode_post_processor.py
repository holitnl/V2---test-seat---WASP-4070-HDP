...
# In each modifier definition you can specify:
#   - "modifier_type": "2D" or "3D"
#   - "offset": for 2D, a [dx, dy] list; for 3D, a [dx, dy, dz] list.
modifier_defs = [
    {"filename": "modifier1.stl", "modifier_type": "3D", "center_multiplier": 1.5, "edge_multiplier": 1.5,
     "gradient_exponent": 1.0, "min_layer": 0.0, "offset": [0, 0]},  
    {"filename": "modifier2.stl", "modifier_type": "3D", "center_multiplier": 2, "edge_multiplier": 2,
     "gradient_exponent": 2, "min_layer": 0.0, "r_eff_scale": 1.0, "offset": [0, 0]}  # If only 2 numbers are given, z offset defaults to 0.
]
...
for mod_def in modifier_defs:
    ...
    if os.path.exists(mfile):
        if mod_type == "2D":
            ...
            # If an offset is provided (2D: [dx, dy]), shift the polygon.
            if "offset" in mod_def:
                dx, dy = mod_def["offset"]
            ...
            # If an offset is provided (3D: [dx, dy, dz]), shift the vertices and centroid.
            if "offset" in mod_def:
                offset = np.array(mod_def["offset"])
                # If offset has only 2 elements, append 0 for z.
                if offset.shape[0] == 2:
                    offset = np.array([offset[0], offset[1], 0])
                mesh_info["vertices"] = mesh_info["vertices"] + offset
                cx, cy, cz = mesh_info["centroid"]
                mesh_info["centroid"] = (cx + offset[0], cy + offset[1], cz + offset[2])
            mod_params = {
                "modifier_type": "3D",
                "mesh": mesh_info["mesh"],
                ...
                "gradient_exponent": mod_def.get("gradient_exponent", 1.0),
                "min_layer": mod_def.get("min_layer", 0.0),
                "r_eff_scale": mod_def.get("r_eff_scale", 1.0)
            }
        else:
...
for mod in modifiers:
    if mod.get("modifier_type", "2D") == "2D":
        ...
        # Also plot effective XY circle at the reference Z.
        cx, cy, cz = mod["centroid_3d"]
        if z_ref >= cz - mod["r_max"] and z_ref <= cz + mod["r_max"]:
            r_eff = mod.get("r_eff_scale", 1.0) * math.sqrt(max(0, mod["r_max"]**2 - (z_ref - cz)**2))
            theta = np.linspace(0, 2*math.pi, 100)
            x_circle = cx + r_eff * np.cos(theta)
            y_circle = cy + r_eff * np.sin(theta)
            ax.plot(x_circle, y_circle, zs=z_ref, zdir="z", color="cyan", linewidth=2, label="3D Modifier (Effective)")
ax.legend()
set_axes_equal(ax)
